...
import numpy as np
from scripts.config import W_PULL, TEMPERATURE_CON, BETA_IB, NUM_CLASSES, BATCH_SIZE, PRE_TRAIN_EPOCHS, MODEL_FILENAME, LR, WORKERS, LATENT_DIM_U, LATENT_DIM_C, NUM_CLASSES, LAMBDA, GAMMA, FINE_TUNE_EPOCHS, dataset_name , CHANNELS, TOLERANCE, UPDATE_INTERVAL, IHMC
import torch
...
def Show_settings():
    ...
    plt.text(0.1, 0.9, 'Pre train Epochs: %.1f' %(PRE_TRAIN_EPOCHS))
    plt.text(0.1, 0.8, 'Fine-tune Epochs: %.1f' %(FINE_TUNE_EPOCHS))
    plt.text(0.1, 0.7, 'Update Interval: %.3f' %(UPDATE_INTERVAL))
    plt.text(0.1, 0.6, 'Common Embeding Dimension: %.3f' %(LATENT_DIM_C))
    plt.text(0.1, 0.5, 'Unique Embeding Dimension: %.3f' %(LATENT_DIM_U))
    plt.text(0.1, 0.4, 'LAMBDA: %.3f' %(LAMBDA))
    plt.text(0.1, 0.3, 'TEMPERATURE_CON: %.3f' %(TEMPERATURE_CON))
    plt.text(0.1, 0.2, 'GAMMA: %.3f' %(GAMMA))
    plt.text(0.1, 0.1, 'W_PULL: %.3f' %(W_PULL))
...
#*****************************************************************************
def Show_dataloader_data(img1, img2, labels, max_show=4):
    """
    img1, img2: [B, C, H, W] torch tensors
    labels:     [B] torch tensor or list-like
    """
    # Basic shape/info prints
    print("Batch shape:", tuple(img1.shape))
    B = img1.shape[0]
    C = img1.shape[1] if img1.ndim == 4 else 1
    H = img1.shape[-2]
    W = img1.shape[-1]
    print(f"Data is {H}x{W}")
    print("Data Max value is:", torch.max(img1).item())
    print("Data Min value is:", torch.min(img1).item())

    n = min(B, max_show)
    cols = 2 if img2 is not None else 1

    # Create an n×cols grid: each row = one sample, columns = img1 / img2
    fig, ax = plt.subplots(n, cols, figsize=(4 * cols, 3 * n))
    # Normalize ax to a 2D array for consistent indexing
    if n == 1 and cols == 1:
        ax = np.array([[ax]])
    elif n == 1:
        ax = np.array([ax])
    ...
    def _show_one(ax_cell, x):
        # x: [C,H,W]
        x = x.detach().cpu()
        if C == 1:
            ax_cell.imshow(x[0].numpy(), cmap='gray')
        elif C == 3:
            ax_cell.imshow(x.permute(1, 2, 0).numpy())
        else:
            # Fallback: visualize first channel
            ax_cell.imshow(x[0].numpy(), cmap='gray')
        ax_cell.axis("off")

    for i in range(n):
        _show_one(ax[i, 0], img1[i])
        if labels is not None:
            try:
                ax[i, 0].set_title(int(labels[i]))
            except Exception:
                ax[i, 0].set_title(str(labels[i]))
        if img2 is not None:
            _show_one(ax[i, 1], img2[i])
            if labels is not None:
                try:
                    ax[i, 1].set_title(int(labels[i]))
                except Exception:
...
def Show_Representation(decoded_common, label):
    fig, ax = plt.subplots(1, NUM_CLASSES, figsize=(10, 4))
    if CHANNELS == 1:
...
def Show_Embedding_Space(z1, z2, labels):
    ...
    plot_projection(X_tsne, labels, NUM_CLASSES)
    plt.title("Unique Embedding Space", fontsize=28)  
...
def Show_Partial_Embedding_Space(z, y_true):
    ...
    if IHMC:
        plt.savefig("Unique Embeded Space.png")
    plt.show() 
#*****************************************************************************        
